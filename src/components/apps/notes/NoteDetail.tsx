...
const NoteDetail = () => {
  const noteDetails = useSelector(
    (state : any) => state.notesReducer.notes[state.notesReducer.notesContent - 1],
  );
  const id = useSelector((state : any) => state.notesReducer.notes.length + 1);

  const dispatch = useDispatch();

  const handleNote = (e : any) => {
    e.preventDefault();
    dispatch(addNote({id, title: 'dummy Title', color: 'primary'}));
  };
        ...
        <div className="p-4">
          <FormGroup>
            ...
            <Input
              ...
              onChange={(e) => dispatch(UpdateNote({id: noteDetails.id, field: 'title', value:e.target.value}))}
            />
          </FormGroup>
          <br />
          ...
          <div className="button-group">
            {colorsBg.map((colorbg) => (
              <Button
                ...
                key={colorbg.bg}
                size="sm"
                onClick={() => dispatch(UpdateNote({id :noteDetails.id, field: 'color', color: `${colorbg.bg}`}))}
              >
