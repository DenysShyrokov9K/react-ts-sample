...
const TicketListing = () => {
    ...
    switch (filter) {
      case 'total_tickets':
        return tickets.filter(
          (c : any) => !c.deleted && c.ticketTitle.toLocaleLowerCase().includes(ticketSearch),
        );
  ...
  const tickets = useSelector((state : any) =>
    getVisibleTickets({
      ...
      filter : state.ticketReducer.currentFilter,
      ticketSearch : state.ticketReducer.ticketSearch,
     }
    ),
  );
