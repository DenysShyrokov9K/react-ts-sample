...
const TicketFilter = () => {
  ...
  const counter = useSelector((state : any) => state.ticketReducer.tickets);
  const pendingC = counter.filter((t : any) => t.Status === 'Pending').length;
  const openC = counter.filter((t : any) => t.Status === 'Open').length;
  const closeC = counter.filter((t : any) => t.Status === 'Closed').length;
  return (
    <Row>
      <Col lg="3">
        <Card
